...
for i in range(len(members)):
  ...
  for j in range(len(fhours1)):
    if i==0:
      ...
      grbind = pygrib.index('/home/gefs/gefs/model_data/gefs/prod/gefs.'+str(ymd)+'/'+str(hour).zfill(2)+'/atmos/pgrb2bp5/ge'+members[i]+'.t'+str(hour).zfill(2)+'z.pgrb2b.0p50.f'+str(fhours1[j]).zfill(3),'name')
      ...
      grbind = pygrib.index('/home/gefs/gefs/model_data/gfs/prod/gfs.'+str(ymd)+'/'+str(hour).zfill(2)+'/atmos/gfs.t'+str(hour).zfill(2)+'z.pgrb2.0p50.f'+str(fhours1[j]).zfill(3),'name')
      #for grb in grbs:
      precip=(grbind.select(name='MSLP (Eta model reduction)')[0].values/100.)
      precip=np.asarray(precip[::-1,:])
      lats,lons = grbind.select(name='2 metre dewpoint temperature')[0].latlons()
